...
data.player_sc = SpriteCollection:new("player")
data.player_sc:init({Sprite:new("img/wizard_idle-Sheet0.png", true, "idleEmpty", 18, 18, Vector:new(7, 9), false, {0.5, 0.1, 0.06, 0.1, 0.1, 0.1}),
...
-- MONSTERS
data.troll_sc = SpriteCollection:new("troll")
data.troll_sc:init({Sprite:new("img/troll_idle-Sheet.png", true, "idle", 16, 16, Vector:new(7, 6)),
    Sprite:new("img/troll_run-Sheet.png", true, "run", 16, 16, Vector:new(7, 6), false, {0.12, 0.12, 0.12, 0.12})})

data.rhino_sc = SpriteCollection:new("rhino")
data.rhino_sc:init({Sprite:new("img/rhino_idle-Sheet.png", true, "idle", 16, 16, Vector:new(6, 13)),
...
-- STAFFS
data.cool_staff_sc = SpriteCollection:new("item")
data.cool_staff_sc:init({Sprite:new("img/magic_staff.png", false, "idle", 16, 16, Vector:new(7, 6))})

data.cool_staffHF = HitboxFactory:new(
    {"hitbox", {item=true}, 3, 14, Vector:new(-5, -5)}
)

data.simple_staff_sc = SpriteCollection:new("item")
data.simple_staff_sc:init({Sprite:new("img/simple_staff.png", false, "idle", 16, 16, Vector:new(7, 6))})

data.simple_staffHF = HitboxFactory:new(
    {"hitbox", {item=true}, 3, 10, Vector:new(-6, -5)}
)

data.gold_staff_sc = SpriteCollection:new("item")
data.gold_staff_sc:init({Sprite:new("img/gold_staff.png", false, "idle", 16, 16, Vector:new(7, 6))})

data.gold_staffHF = HitboxFactory:new(
    {"hitbox", {item=true}, 3, 14, Vector:new(-5, -5)}
)

data.power_staff_sc = SpriteCollection:new("item")
data.power_staff_sc:init({Sprite:new("img/power_staff.png", false, "idle", 16, 16, Vector:new(7, 6))})
